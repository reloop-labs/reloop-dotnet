...
namespace Reloop.Services
{
    public class ApiKeyService
    {
        ...
        {
            return _client.FetchAsync<ApiKeyWithKey>(HttpMethod.Post, "/api-key/v1/", @params);
        }
        ...
        {
            return _client.FetchAsync<ApiKey>(HttpMethod.Get, $"/api-key/v1/{id}");
        }

        public Task<ApiKey?> UpdateAsync(string id, UpdateApiKeyParams @params)
        {
            return _client.FetchAsync<ApiKey>(new HttpMethod("PATCH"), $"/api-key/v1/{id}", @params);
        }

        public Task<DeleteApiKeyResponse?> DeleteAsync(string id)
        {
            return _client.FetchAsync<DeleteApiKeyResponse>(HttpMethod.Delete, $"/api-key/v1/{id}");
        }

        public Task<ApiKeyWithKey?> RotateAsync(string id)
        {
            return _client.FetchAsync<ApiKeyWithKey>(HttpMethod.Post, $"/api-key/v1/rotate/{id}");
        }

        public Task<ApiKey?> EnableAsync(string id)
        {
            return _client.FetchAsync<ApiKey>(HttpMethod.Post, $"/api-key/v1/enable/{id}");
        }

        public Task<ApiKey?> DisableAsync(string id)
        {
            return _client.FetchAsync<ApiKey>(HttpMethod.Post, $"/api-key/v1/disable/{id}");
        }
    }
}
